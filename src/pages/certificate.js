...
const Certificate = () => {
  ...
  const verify = () => {
    if (!_.isNil(attendees[orderNo])) {
      ...
      setValidMessage(
        <p className="red text-center">Order No. {orderNo} is invalid.</p>
      );
    }
  };
  ...
  useEffect(() => {
    if (generate) {
      ...
      doc.save(`${certName} - Certificate of Attendance.pdf`);
    }

    setGenerate(false);
  }, [generate, setGenerate, certName]);

  // [UI] Input the order number first
  if (!isValidOrderNo) {
    return (
      <>
  ...
  // [UI] Preview the certificate and let generate
  return (
    <>
      <SEO title="Certificate of Attendance" />
      <section>
